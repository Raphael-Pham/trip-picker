...
// Captured flight prices keyed by departure airport → destination region.
// Generated by scripts/captureFlightPrices.ts. Falls back to REGION_FLIGHT_COSTS.
let capturedPrices: Record<string, Record<string, number>> | null = null;
try {
  // Dynamic require so this gracefully does nothing if the file doesn't exist yet.
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  capturedPrices = require('../data/flightPrices.json');
} catch {
  // File not generated yet — regional matrix is used exclusively.
}
...
export function estimateFlightCost(
  ...
  for (const code of destinationAirportCodes) {
    ...
    // Prefer per-airport captured price; fall back to regional matrix
    const captured = capturedPrices?.[dep]?.[destRegion];
    const baseCost = (captured != null && captured > 0)
      ? captured
      : (REGION_FLIGHT_COSTS[depRegion]?.[destRegion] ?? 800);
